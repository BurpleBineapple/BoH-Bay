...
/obj/structure/missile
	...
	icon = 'modular_boh/icon/obj/bigmissile.dmi'
	icon_state = "base"
...
/obj/structure/missile/proc/activate()
	...
	overmap_missile = new /obj/overmap/projectile(null, start_object.x, start_object.y)
...
/obj/structure/missile/proc/enter_level(var/z_level, var/target_fore_dir, var/target_dir)
	...
	if(!heading)
		heading = random_missile_dir() // To prevent the missile from popping into the middle of the map and sitting there

	var/start_x = min(world.maxx / 2) + rand(-10, 10)//MIN = Floor
	var/start_y = min(world.maxy / 2) + rand(-10, 10)
